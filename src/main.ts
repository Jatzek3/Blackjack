...
class Manager{
    ...
    initialization() {
            ...
            let singleOrMulti:any/*add alias*/ = prompt('Do you want to play alone, or with others(enter number(max8))', 'yes')
...
class Game {
    ...
    checkForWinnerOnStart() {
        const playersLeft:Player[] = this.Players.filter(player => player.score< 22)
        if (playersLeft.length === 1){
            ...
            const winningPlayer:Player[] = this.Players.filter((player)=> player.playerWon)
