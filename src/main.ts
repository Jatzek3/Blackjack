let dealerNode: HTMLDivElement = document.querySelector('.dealer-button');
let playersNode: HTMLDivElement = document.querySelector('.players');

let shuffleReady;
let turnStarted: Function;
let actualGame: Game;

type strOrInt = number | string;
...
class Manager {
	...
	initialization() {
		...
		const regex: RegExp = new RegExp(validName);
	...
	replay() {
		...
		let decision: string = prompt('Do you want to play a new game?', 'yes');
		if (decision || '') {
...
class Game {
	...
	checkForWinnerOnStart() {
		const playersLeft: Player[] = this.Players.filter(
		...
		if (playersLeft.length === 1) {
			...
			const winningPlayer: Player[] = this.Players.filter(
				(player) => player.playerWon,
			);
	...
	checkForWinnerOnGameEnd() {
		let playersLeft: Player[] = this.Players.filter(
			(player) => player.score < 22,
		);
...
class Dealer {
	...
	dealerDraws() {
		const dealerCardsUl: HTMLUListElement = document.querySelector(
			`.${this.name}-cards`,
		);
		let dealerScore: HTMLParagraphElement = document.querySelector(
			'.Dealer-score',
		);
	...
	createPlayer(name = this.name) {
		...
		let cards: HTMLUListElement = document.createElement('ul');
...
class Player extends Dealer {
	...
	hitMeHandle() {
		const playerCardsUl: HTMLDivElement = document.querySelector(
			`.${this.name}-cards`,
		);
		const playerScore: HTMLDivElement = document.querySelector(
			`.${this.name}-score`,
		);
	...
	startTurn(name = this.name) {
		...
		let playerScore: HTMLParagraphElement = document.querySelector(
			`.${name}-score`,
		);
		let hitMeButton: HTMLButtonElement = document.querySelector(
			`.${name}-hit-button`,
		);
		let passButton: HTMLButtonElement = document.querySelector(
			`.${name}-fold-button`,
		);
